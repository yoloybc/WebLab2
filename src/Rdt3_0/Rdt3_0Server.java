...
public class Rdt3_0Server {
    ...
    public Rdt3_0Server() throws Exception{
        model = new Model();
    ...
    public void receiveFile() throws Exception{
        ...
        if(Integer.parseInt(received.substring(10, 11).trim()) == sed_ack){
            String filename="E:/javacode/WebLab2/src/ServerFile/ClientFile"+sed_ack+".txt";
            File file=new File(filename);
            file.createNewFile();
            ...
            writer.flush();
            sendAck(sed_ack);
            System.out.println("接收"+received+"向客户端返回ack:" + sed_ack);
            sed_ack++;
        }else{
            return;
        }
    ...
    public void timeOut() throws IOException {
        String filename="E:/javacode/WebLab2/src/ServerFile/ServerFile"+rcv_ack+".txt";
        File file=new File(filename);
        FileInputStream fis = new FileInputStream(file);
